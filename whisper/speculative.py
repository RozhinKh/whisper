...
def speculative_transcribe(
    ...
    # IMPORTANT: hooks are installed *after* embed_audio so the encoder is not
    # affected, and the sanity check below runs *before* hooks so it cannot
    # accidentally write into the cache.
